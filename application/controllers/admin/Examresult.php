...
class Examresult extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data['exam_id']       = $exam_id;
            $data['exam_group_id'] = $exam_group_id;
            // Fetch actual exam date from exam_schedules
            $exam_schedule = $this->db->get_where('exam_schedules', ['exam_id' => $exam_id])->row();
            $data['exam_date'] = ($exam_schedule && !empty($exam_schedule->date_of_exam)) ? $exam_schedule->date_of_exam : date('Y-m-d');
        }
    ...
    public function sendwhatsapp()
    {
        ...
        if (empty($students)) {
            echo json_encode(['status' => 0, 'message' => 'No students selected']);
            return;
        }
    
        // Get WhatsApp template from notification_setting
        $template_row = $this->db->get_where('notification_setting', ['type' => 'exam_result'])->row();
        $template = $template_row ? $template_row->template : null;
    
        // Fallback default template if none set
        if (empty($template)) {
        ...
        foreach ($students as $student) {
            $phone = preg_replace('/[^0-9]/', '', $student['guardian_phone'] ?? '');
            ...
            $student['exam_date'] = $this->input->post('exam_date');
            $message = $this->whatsappgateway->buildMessage(
                $template,
                $student,
                $exam_name,
                $class_name,
            ...
            // Send message
            $success = $this->whatsappgateway->sendDirectMessage($phone, $message);
        ...
        echo json_encode([
            ...
            'failed'  => $failed,
            'results' => $results,
            'message' => "Messages sent: {$sent} success, {$failed} failed."
        ]);
    }
    public function getWhatsAppTemplate()
    {
        $type = $this->input->post('type') ?? 'exam_result';
        $row  = $this->db->get_where('notification_setting', ['type' => $type])->row();
    ...
    public function saveWhatsAppTemplate()
    {
        ...
        // Check if row exists
        $exists = $this->db->get_where('notification_setting', ['type' => $type])->row();
        ...
        if ($exists) {
            ...
            $this->db->insert('notification_setting', [
                'type'        => $type,
                'template'    => $template,
                'is_whatsapp' => 1
            ]);
