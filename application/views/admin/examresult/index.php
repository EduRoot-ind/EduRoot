                                ...
                                foreach ($studentList as $wa_idx => $sv):
                                    ...
                                    if (isset($subjectList) && !empty($subjectList)) {
                                        foreach ($subjectList as $sub) {
                                            ...
                                            $date_val = '';
                                            if (!empty($sub->date_from) && $sub->date_from !== '0000-00-00') {
                                                $ts = strtotime($sub->date_from);
                                                if ($ts !== false) { $date_val = date('d-m-Y', $ts); }
                                            }
                                            $marks_arr[] = [
...
<script type="text/javascript">
    ...
    var waExamDate       = '<?php echo isset($exam_date) ? $exam_date : date("Y-m-d"); ?>';
    var waExamDate       = '<?php echo isset($exam_date) ? $exam_date : date('Y-m-d'); ?>';
    ...
    function waBuildMsg(tpl, data) {
        ...
        if (data.marks && data.marks.length) {
            $.each(data.marks, function(i, m) {
                var datePrefix = m.date ? 'dt:' + m.date + ' | ' : '';
                subMarks += '  • ' + datePrefix + m.subject + ': ' + m.marks + '/' + m.max + '\n';
    ...
    function waRefreshPreview() {
        ...
        var msg = waBuildMsg(tpl, {
            ...
            exam_date: $('#waExamDate').val() || waExamDate,
            marks: [
                {subject:'English', marks:22, max:25, date:'24-03-2026'},
                {subject:'Maths',   marks:20, max:25, date:'25-03-2026'},
                {subject:'Hindi',   marks:18, max:25, date:'26-03-2026'},
                {subject:'EVS',     marks:21, max:25, date:'27-03-2026'}
            ]
        });
